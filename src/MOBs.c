...
Variable_stats *s_enemies_variable_stats ()
{
  ...
  *s_variables = {{'C',"Carambolas", 0, 0, 0},
                  {'J',"JMotherFucker", 0, 0, 0 }};
...
Variable_stats *g_enemies_variable_stats ()
{
  ...
  *g_variables = {{'I',"Imtired", 0, 0, 0},
                  {'G',"Godblastmetodeath", 0, 0, 0 }};

  return g_variables; 
}

void init_enemy (int enemies_size, Enemies *enemies, Tag *tag, Variable_stats d_variables, Variable_stats s_variables, Variable_stats d_variables)
{
  int current_enemy = 0;
  ...
  for (; current_enemy < dumb_enemies; current_enemy ++)
  {
    ...
    init_enemy_stats (enemy[current_enemy], tag[current_enemy], d_variables[rand() % D_ENEMIES]);
  }
  free (d_variables);
  ...
  for (; current_enemy < dumb_plus_smart; current_enemy ++)
  {
    ...
    init_enemy_stats (enemy[current_enemy], tag[current_enemy], s_variables[rand() % S_ENEMIES]);
  }
  free (s_variables);
  ...
  for (; current_enemy < enemies_size ; current_enemy ++)
  {
    ...
    init_enemy_stats (enemy[current_enemy], tag[current_enemy], g_variables[rand() % G_ENEMIES]);
  }
  free (g_variables);
}
