...
* Displays the vision of the character on the screen according some restrictions
* like step up above the enemies or eliminate the charactes that define the differents
* lands. It also save the path that the character have discovered already on the array
...
void vision (WINDOW *main_window, Character *character, int map_height, int map_width, char map[][map_width], char traveled_path[][map_width])
{
  ...
  for (int x = x_min; x <= x_max; x++) {
    for (int y = y_min; y <= y_max; y++) {
      ...
      /*range of vison of character*/
      else if (pow(x - character->x, 2) + pow(y - character->y, 2) <= pow(range, 2)) {
        /*DEFINE CASES*/
        if (traveled_path[y][x] == ENEMY_G || traveled_path[y][x] == ENEMY_S || traveled_path[y][x] == ENEMY_O ){
